...
const inputClass = 'h-12 w-full rounded-2xl border border-[var(--editable-border)] bg-[#f0f7fb] px-4 text-sm font-medium text-[#1a2a3a] outline-none transition placeholder:text-[#8eaab8] focus:border-[#e8788a]'
const buttonClass = 'inline-flex h-12 w-full items-center justify-center rounded-2xl bg-gradient-to-r from-[#e8788a] to-[#d4627a] px-6 text-sm font-bold text-white shadow-[0_2px_12px_rgba(232,120,138,0.3)] transition hover:-translate-y-0.5 hover:shadow-[0_4px_20px_rgba(232,120,138,0.4)] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-[#e8f4f8] text-[#2bbdc5]' : 'bg-[#fdeef0] text-[#e8788a]'}`}>{message}</p> : null}
